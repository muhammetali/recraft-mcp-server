...
Releases from 1.3.1 onward are generated by semantic-release and published as
[GitHub Releases](https://github.com/muhammetali/recraft-mcp-server/releases),
with notes derived from the commit messages. The entries below are the
hand-written history up to that point, kept because they explain decisions the
commit log alone does not.

The version in `package.json` reads `0.0.0-development` on purpose: the
released version is decided at release time from the commits, so a number
checked into the repository would only ever be a stale copy of it.
