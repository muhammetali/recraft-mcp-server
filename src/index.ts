...
const responseFormatSchema = z.enum(['url', 'b64_json']).default('url').describe('Response format');
...
const textLayoutSchema = z
  ...
  .optional()
  .describe('Text placement on image');

const controlsSchema = z
  .object({
    colors: z
      .array(z.array(z.number().int().min(0).max(255)))
      .optional()
    ...
    background_color: z
      ...
      .optional()
      .describe('Background color RGB (0-255)'),
    artistic_level: z
      .number()
      .int()
      .min(0)
    ...
    no_text: z.boolean().optional().describe('Exclude text layouts (V3 only)'),
  })
  .optional()
...
server.registerTool(
  ...
  {
    description: 'Check your Recraft account info and remaining API credits',
...
server.registerTool(
  'recraft_vectorize',
  {
    description: 'Convert a raster image (PNG/JPG/WEBP) to SVG vector format.',
    inputSchema: z.object({
...
server.registerTool(
  'recraft_erase_region',
  {
    ...
    inputSchema: z.object({
      ...
      mask_path: z
        .string()
        .describe('Local path to mask (white=erase, black=preserve, same dimensions)'),
      response_format: responseFormatSchema,
    }),
  },
  ...
  'recraft_batch_generate',
  {
    ...
    inputSchema: z.object({
      ...
      substyle: substyleSchema,
        ...
        .min(1)
        .describe('Array of assets to generate'),
    }),
  },
  async ({ assets }) => {
    try {
  ...
  'recraft_generate_themed_set',
  {
    ...
    inputSchema: z.object({
      ...
      prompt_suffix: z
        .string()
        .describe(
          'Suffix appended to every symbol prompt for consistent style (e.g., ", ultra detailed 3D render, polished metallic surface, dramatic volumetric lighting from above, centered composition on pure black background, no text, no letters, sharp focus, cinematic lighting")',
        ),
      symbols: z
        .array(
          z.object({
            ...
            prompt_detail: z
              .string()
              .describe('Symbol-specific prompt prefix (e.g., "A golden scarab beetle amulet")'),
          }),
        )
        .min(1)
        .describe('Array of symbols to generate'),
      output_dir: z
        .string()
        .describe('Base output directory (symbols/ and ui/ subdirs will be created)'),
      size: sizeSchema,
      remove_bg: z
        .boolean()
        .default(true)
        .describe('Remove background from symbols (default: true)'),
      bg_prompt: z
        .string()
        .optional()
        .describe('Background image prompt (if omitted, no background is generated)'),
      bg_size: z
        .string()
        .default('1344x768')
        .describe('Background image size (default: 1344x768 / 16:9)'),
      generate_manifest: z
        .boolean()
        .default(true)
        .describe('Generate asset-manifest.json for Pixi.js integration'),
    }),
  },
  async (params) => {
    try {
  ...
  'recraft_generate_sized',
  {
    ...
    inputSchema: z.object({
      ...
      width: z
        ...
        .min(1)
        .max(4096)
        .describe('Target width in pixels (e.g., 64, 128, 256)'),
      height: z
        .number()
        .int()
        .min(1)
        .max(4096)
        .describe('Target height in pixels (e.g., 64, 128, 256)'),
      output_path: z.string().describe('Where to save the final image'),
      fit: z
      ...
      remove_bg: z
        .boolean()
        .default(false)
        .describe('Remove background before resize (default: false)'),
    }),
  },
  async (params) => {
    try {
  ...
  'recraft_compare_styles',
  {
    ...
    inputSchema: z.object({
      ...
      styles: z
        ...
        .max(10)
        .describe(
          'Array of style names to compare (e.g., ["digital_illustration", "pixel_art", "vector_art"])',
        ),
      ...
      grid: z.boolean().default(true).describe('Generate a comparison grid image (default: true)'),
      grid_columns: z
        .number()
        .int()
        .min(1)
  ...
  'recraft_outpaint',
  {
    ...
    inputSchema: z.object({
      ...
      size: z
        ...
        .optional()
        .describe(
          'Target size for the resulting image — the source image is placed inside it. Cannot combine with expand_*.',
        ),
      expand_left: z
        .number()
        .int()
        ...
        .max(4096)
        .optional()
        .describe('Pixels to add to the left. Cannot combine with size.'),
      expand_right: z
        .number()
        .int()
        .min(0)
        .max(4096)
        .optional()
        .describe('Pixels to add to the right. Cannot combine with size.'),
      expand_top: z
        .number()
        .int()
        .min(0)
        .max(4096)
        .optional()
        .describe('Pixels to add to the top. Cannot combine with size.'),
      expand_bottom: z
        .number()
        .int()
        .min(0)
        .max(4096)
        .optional()
        .describe('Pixels to add to the bottom. Cannot combine with size.'),
      zoom_out_percentage: z
        ...
        .optional()
        .describe(
          'Scales the source image down by this percentage before outpainting, revealing more surrounding context [0-100). Can combine with size or expand_*.',
        ),
      model: z
        .string()
        .default('recraftv3')
  ...
  'recraft_explore',
  {
    ...
    inputSchema: z.object({
      ...
      model: z
        ...
        .default('recraftv4_1')
        .describe(
          'Model: recraftv4_1, recraftv4_1_vector, recraftv4_1_pro, recraftv4_1_pro_vector, recraftv4, recraftv4_vector, recraftv4_pro, recraftv4_pro_vector',
        ),
  ...
  'recraft_explore_similar',
  {
    ...
    inputSchema: z.object({
      ...
      similarity: z
        ...
        .min(1)
        .max(5)
        .describe('How closely to match the source image (1-5)'),
      response_format: responseFormatSchema,
    }),
  },
  async (params) => {
