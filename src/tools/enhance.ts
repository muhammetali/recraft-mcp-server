...
import { validateFilePath, validateResponseFormat } from '../validation.js';
...
export async function eraseRegion(
  ...
  const fileBuffer = readFileSync(filePath);
  formData.append('image', new Blob([fileBuffer], { type: 'image/png' }), basename(filePath));

  const maskBuffer = readFileSync(maskPath);
  formData.append('mask', new Blob([maskBuffer], { type: 'image/png' }), basename(maskPath));
